...
public class ModuleBlockRender extends ModuleRender {
    ...
    String getSafeItemName(int id, int meta) {
    	try {
    		return new ItemInstance(id, 1, meta).getTranslationKey();
    	}catch (NullPointerException e) {
    		try {
    			return new ItemInstance(id, 1, 0).getTranslationKey();
    ...
    public void updateBlock(int blockX, int blockY, int blockZ) {
        ...
        this.stringBlockProperties = TranslationStorage.getInstance().method_995(getSafeItemName(currentBlockID, currentBlockMeta)).trim() +  " " + currentBlockID + ":" + currentBlockMeta + " ";
        this.stringBlockHardness = "H: " + BlockBase.BY_ID[currentBlockID].getHardness();
        
        this.colourType = getColorByHardness(BlockBase.BY_ID[currentBlockID].getHardness());
    }
    
    public void __updateBlockDebug() {
    	Random random = new Random();
    	
    	this.currentBlockID = BlockBase.WOOL.id;
    ...
    public void doBlockRendering(int x, int y, int anchor) {
        ...
        if(!_debug) {
	        ...
	        for(int q = 0; q < vals.length; q++)
	        	vals[q] = blockDescPlugins[q].getAdditionalBlockDescription(currentBlockID, currentBlockMeta, minecraft.level, minecraft.hitResult.x, minecraft.hitResult.y, minecraft.hitResult.z);
        ...
        if(config.showBlockModernStyle && !config.transparency)
            realXY = RenderUtils.modernRenderByAnchor(x, y, Math.max(textRenderer.getTextWidth(stringBlockCoordinates), textRenderer.getTextWidth(stringBlockProperties + stringBlockHardness)) + 33, 23 + (f * 10), anchor);
        else
        	realXY = RenderUtils.gradientRenderByAnchor(x, y, Math.max(textRenderer.getTextWidth(stringBlockCoordinates), textRenderer.getTextWidth(stringBlockProperties + stringBlockHardness)) + 36, 26 + (f * 10), config.showBlockRGB[0], config.showBlockRGB[1], anchor, config.transparency);

        boolean e = false;
        if(!_debug) {
	        for(IOverrideBlockRender g : renderOverridePlugins) {
	        	boolean j = g.overrideHUDItemRenderer(currentBlockID, currentBlockMeta, minecraft.level, itemRenderer, realXY[0] + 5, realXY[1] + 5, minecraft.hitResult.x, minecraft.hitResult.y, minecraft.hitResult.z);
	        	if(!e) e = j;
	        }
        }
        
        if(!e)
        	RenderUtils.renderItem(itemRenderer,textRenderer, minecraft.textureManager, new ItemInstance(this.currentBlockID, 1, this.currentBlockMeta), realXY[0] + 5, realXY[1] + 5);
        
        textRenderer.drawText(stringBlockCoordinates, realXY[0] + 25, realXY[1] + 5, config.showBlockRGB[2]);
        textRenderer.drawText(stringBlockProperties, realXY[0] + 25, realXY[1] + 15, config.showBlockRGB[2]);;
        textRenderer.drawText(stringBlockHardness, realXY[0] + 25 + textRenderer.getTextWidth(stringBlockProperties), realXY[1] + 15, colourType == 0 ? 0x55FFFF : (colourType == 1 ? 0x55FF55 : (colourType == 2 ? 0xFFFF00 : 0xFF0000)));
        ...
        if(!_debug) {
	        ...
	        for(int q = 0; q < vals.length; q++) {
	        	if(vals[q] == null)
	        		continue;
	        	for(String h : vals[q]) {
	                textRenderer.drawText(h, realXY[0] + 25, realXY[1] + 25 + (f * 10), config.showBlockRGB[2]);
	                f++;
